...
async function main() {
  ...
  const optionValue = "ORLNON06_2173A"; // The value you want to select
  ...
  await iframe.select(
    ...
    `${SalesForce.siteOptions[optionValue]}`
  );
  ...
  const table = await iframe.$(SalesForce.tableSelector);
    ...
    while (hasNextPage) {
      ...
      await iframe.waitForSelector(SalesForce.tableSelector);
      const table = await iframe.$(SalesForce.tableSelector);
      const tableData = await iframe.evaluate((table) => {
        const rows = Array.from(table.querySelectorAll("tbody.ng-scope tr"));
        return rows.map((row) => {
          const columns = row.querySelectorAll("td");

          // treat data, createa a street, house number, from address
          const address = columns[0].innerText.trim();
          const addressArray = address.split(" ");
          const houseNumber = addressArray[0];
          // street is the rest of the array
          const streetName = addressArray[1]
          const consent =
            columns[3].innerText.trim() ?? columns[1].innerText.trim();
          return {
            address: columns[0].innerText.trim(),
            streetName: streetName,
            houseNumber: houseNumber,
            statusAttempt: `${columns[1].innerText.trim()} + ${columns[3].innerText.trim()}`,
            status: columns[1].innerText.trim(),
            dsa: columns[2].innerText.trim(),
            consent: consent,
            followUp: columns[4].innerText.trim(),
    ...
    fs.writeFile("allData.json", JSON.stringify(allData, null, 2), (err) => {
      if (err) {
